...
export default function ProfileScreen() {
    ...
    // If ID is provided and different, it's a public view
    const isOwnProfile = !id || (user && id === user.id);
    const viewUserId = isOwnProfile ? user?.id : (id as string);
        ...
        try {
            if (isOwnProfile) {
                const [userProfile, userMembership, userStats, userHistory, userFamily, userLevel, userCommunities] = await Promise.all([
                    userService.getUserProfile(viewUserId),
                    userService.getMembership(viewUserId),
                    userService.getStats(viewUserId),
                    userService.getCheckinHistory(viewUserId),
                    userService.getFamilyDetails(viewUserId),
                    userService.getLevelProgress(viewUserId),
                    supabase.from('community_members').select('*, communities(*)').eq('user_id', viewUserId)
                ]);
                ...
                // Fetch stats/level if possible (permissions might restrict, handle gracefully)
                try {
                    const publicStats = await userService.getStats(viewUserId);
                    setStats(publicStats);
                } catch (e) { setStats({ totalCheckins: 0, streak: 0 }); }

                try {
                    const publicLevel = await userService.getLevelProgress(viewUserId);
                    setLevelProgress(publicLevel);
                } catch (e) { setLevelProgress(null); }

                try {
                    const { data: publicCommunities } = await supabase.from('community_members').select('*, communities(*)').eq('user_id', viewUserId);
                    setMyCommunities(publicCommunities || []);
                } catch (e) { setMyCommunities([]); }
    ...
    useEffect(() => {
        fetchData();
    }, [viewUserId]); // Re-fetch when viewUserId changes
    ...
    return (
        ...
        >
            ...
            <View style={styles.header}>
                ...
                <Text style={styles.email}>@{profile?.username || 'user'}</Text>
            </View>
