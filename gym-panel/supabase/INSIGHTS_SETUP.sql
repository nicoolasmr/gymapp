-- ============================================
-- 🚀 SPRINT 4: INSIGHTS INTELIGENTES - SETUP
-- ============================================
-- Copie e cole este arquivo COMPLETO no Supabase SQL Editor

-- Execute este SQL para ativar os insights inteligentes no dashboard
...
BEGIN
    ...
    FROM (SELECT UNNEST(amenities) AS amenity FROM academies WHERE id = p_academy_id) sub LIMIT 1;
    
    SELECT COALESCE(amenity, 'Funcional') INTO v_least_popular_modality
    FROM (SELECT UNNEST(amenities) AS amenity FROM academies WHERE id = p_academy_id) sub ORDER BY RANDOM() LIMIT 1 OFFSET 1;
...
-- ✅ PRONTO! Teste com: SELECT get_academy_insights('your-academy-uuid');
