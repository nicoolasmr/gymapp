-- ============================================
-- MIGRATION MVP 0.4 - NOVO MODELO DE NEGÓCIO
-- Data: 23/11/2025
-- ============================================

-- 1. ENUMS E ALTERAÇÕES DE TABELA
-- ============================================

-- Criar tipo de modalidade
CREATE TYPE academy_modality AS ENUM ('gym_standard', 'crossfit_box', 'studio');

-- Adicionar modalidade na tabela de academias
ALTER TABLE academies 
ADD COLUMN IF NOT EXISTS modality academy_modality DEFAULT 'gym_standard';

-- Adicionar modalidade na tabela de planos
ALTER TABLE plans 
ADD COLUMN IF NOT EXISTS modality academy_modality DEFAULT 'gym_standard';

-- Adicionar colunas de controle nos planos
ALTER TABLE plans
ADD COLUMN IF NOT EXISTS weekly_limit INT, -- Para CrossFit (2-3 vezes)
ADD COLUMN IF NOT EXISTS requires_reservation BOOLEAN DEFAULT FALSE; -- Para Studios

-- 2. NOVAS TABELAS
-- ============================================

-- Tabela de Regras de Repasse
...
-- Tabela de Janelas de Horário (CrossFit)
...
CREATE TABLE IF NOT EXISTS reservations (
    ...
    class_id UUID REFERENCES studio_classes(id), -- Pode ser null se for CrossFit window (futuro)
    window_id UUID REFERENCES class_windows(id), -- Opcional para CrossFit
    date DATE NOT NULL,
    status TEXT CHECK (status IN ('reserved', 'checked_in', 'cancelled', 'noshow')) DEFAULT 'reserved',
    created_at TIMESTAMPTZ DEFAULT now(),
    UNIQUE(user_id, class_id, date), -- Não pode reservar a mesma aula 2x
    UNIQUE(user_id, window_id, date)
);

-- Atualizar tabela de Checkins para vincular com reserva
...
-- 3. ATUALIZAR PLANOS (PREÇOS E REGRAS)
-- ============================================

-- Plano 1: Academia Convencional (Solo)
...
price = 149.00, name = 'Plano Gym Solo', modality = 'gym_standard';

-- Plano 1: Academia Convencional (Família)
INSERT INTO plans (id, name, price, max_members, description, modality, weekly_limit, requires_reservation)
VALUES (4, 'Plano Gym Família', 449.00, 4, 'Acesso diário para família em academias convencionais', 'gym_standard', NULL, FALSE)
ON CONFLICT (id) DO UPDATE SET 
price = 449.00, name = 'Plano Gym Família', modality = 'gym_standard';

-- Plano 2: CrossFit Flex (Solo)
INSERT INTO plans (id, name, price, max_members, description, modality, weekly_limit, requires_reservation)
VALUES (5, 'CrossFit Flex Solo', 220.00, 1, '3 treinos por semana em Boxes de CrossFit', 'crossfit_box', 3, FALSE)
ON CONFLICT (id) DO UPDATE SET 
price = 220.00, name = 'CrossFit Flex Solo', modality = 'crossfit_box', weekly_limit = 3;

-- Plano 2: CrossFit Flex (Família)
INSERT INTO plans (id, name, price, max_members, description, modality, weekly_limit, requires_reservation)
VALUES (6, 'CrossFit Flex Família', 700.00, 4, '3 treinos por semana para família em Boxes', 'crossfit_box', 3, FALSE)
ON CONFLICT (id) DO UPDATE SET 
price = 700.00, name = 'CrossFit Flex Família', modality = 'crossfit_box', weekly_limit = 3;

-- Plano 3: Studio Pass (Solo)
INSERT INTO plans (id, name, price, max_members, description, modality, weekly_limit, requires_reservation)
VALUES (7, 'Studio Pass Solo', 300.00, 1, '2 aulas por semana em Studios (Pilates, Yoga, etc)', 'studio', 2, TRUE)
ON CONFLICT (id) DO UPDATE SET 
price = 300.00, name = 'Studio Pass Solo', modality = 'studio', weekly_limit = 2, requires_reservation = TRUE;

-- Plano 3: Studio Pass (Família)
INSERT INTO plans (id, name, price, max_members, description, modality, weekly_limit, requires_reservation)
VALUES (8, 'Studio Pass Família', 1000.00, 4, '2 aulas por semana para família em Studios', 'studio', 2, TRUE)
ON CONFLICT (id) DO UPDATE SET 
price = 1000.00, name = 'Studio Pass Família', modality = 'studio', weekly_limit = 2, requires_reservation = TRUE;


-- 4. RPC: REALIZAR RESERVA (Para Studios)
-- ============================================
...
BEGIN
    ...
    IF _plan_id IS NULL THEN
        RAISE EXCEPTION 'Usuário não possui plano ativo';
    END IF;

    -- 2. Verificar se o plano cobre Studio
    -- (Simplificação: Planos Studio cobrem Studio. Planos Gym não.)
    IF _modality != 'studio' THEN
        RAISE EXCEPTION 'Seu plano não permite reservas em Studios';
    END IF;
    ...
    IF _reservations_this_week >= _weekly_limit THEN
        RAISE EXCEPTION 'Limite semanal de reservas atingido';
    END IF;

    -- 4. Verificar Capacidade da Aula
    ...
    IF _current_reservations >= _class_capacity THEN
...
-- 5. RPC: CHECK-IN INTELIGENTE (ATUALIZADO)
-- ============================================
...
BEGIN
  ...
  IF _academy_lat IS NULL OR _academy_active IS FALSE THEN
    RAISE EXCEPTION 'Academia não encontrada ou inativa';
  END IF;

  -- 2. Validar Distância (GPS - 300m)
  -- Fórmula Haversine simplificada para performance
  _distance := (
    6371 * acos(
  ...
  IF _distance > 300 THEN
    RAISE EXCEPTION 'Você está muito longe da academia (%m). Aproxime-se para fazer check-in.', floor(_distance);
  END IF;

  -- 3. Buscar Plano Ativo do Usuário
  SELECT p.id, p.modality, p.weekly_limit 
  ...
  -- 4. Validação por Modalidade
  
  -- A) GYM STANDARD
  IF _academy_modality = 'gym_standard' THEN
      -- Limite diário: 1 check-in
      SELECT count(*) INTO _checkins_today FROM checkins 
      WHERE user_id = _user_id AND created_at > current_date;
      
      IF _checkins_today > 0 THEN
          RAISE EXCEPTION 'Limite diário de 1 check-in atingido';
      END IF;

  -- B) CROSSFIT BOX
  ELSIF _academy_modality = 'crossfit_box' THEN
      -- Verificar se plano cobre CrossFit (CrossFit cobre CrossFit, Studio cobre tudo, Gym não cobre CrossFit)
      IF _plan_modality = 'gym_standard' THEN
          RAISE EXCEPTION 'Seu plano não inclui CrossFit. Faça um upgrade!';
      END IF;

      -- Limite Semanal
      SELECT count(*) INTO _checkins_week FROM checkins 
      WHERE user_id = _user_id 
      AND created_at >= date_trunc('week', now());

      IF _checkins_week >= COALESCE(_weekly_limit, 3) THEN -- Default 3 se null
          RAISE EXCEPTION 'Limite semanal de treinos atingido para seu plano';
      END IF;

      -- Validar Janela de Horário (Simplificado: Verificar se existe alguma janela ativa AGORA +/- 15min)
      -- (Para implementação completa, precisaria cruzar com a tabela class_windows)
      -- Por enquanto, assumimos que se o box está aberto, ok.

  -- C) STUDIO
  ELSIF _academy_modality = 'studio' THEN
      -- Verificar se plano cobre Studio
      IF _plan_modality != 'studio' THEN
          RAISE EXCEPTION 'Seu plano não inclui Studios. Faça um upgrade!';
      END IF;

      -- Verificar RESERVA para HOJE e AGORA (janela de tolerância)
      SELECT id INTO _reservation_id
      FROM reservations
      WHERE user_id = _user_id 
      AND date = current_date
      AND status = 'reserved'
      -- Aqui poderia validar horário da aula, mas vamos simplificar validando o dia
      LIMIT 1;

      IF _reservation_id IS NULL THEN
          RAISE EXCEPTION 'Você não possui reserva agendada para hoje neste estúdio.';
      END IF;
  ...
  RETURN json_build_object(
    ...
    'modality', _academy_modality
  );
END;
$$;

-- 6. ATUALIZAR ACADEMIA DE TESTE
-- ============================================
UPDATE academies 
SET modality = 'crossfit_box' 
WHERE name = 'Academia Teste MVP 0.3';

-- Criar um Studio de Teste
INSERT INTO academies (name, address, lat, long, active, description, modality)
VALUES (
  'Studio Pilates Zen',
  'Rua das Flores, 123',
  -22.878260606151592, -- Mesma lat para facilitar teste
  -42.04539959521476,
  TRUE,
  'Studio especializado em Pilates e Yoga',
  'studio'
);

-- Criar uma aula para o Studio
